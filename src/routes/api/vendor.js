...
setupGoogleStrategy('/api/vendor/google/callback'); // Default callback for vendor
...
router.get(
  ...
  (req, res) => {
    ...
    jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: '7d' }, (err, token) => {
      if (err) throw err;
      res.redirect(
        `http://${process.env.VENDOR_DOMAIN}.localhost:${process.env.VENDOR_PORT}/login-success?token=${token}`
      );
    });
  }
);
...
router.use('/', userAuthRole([USER_ROLE.VENDOR]));
